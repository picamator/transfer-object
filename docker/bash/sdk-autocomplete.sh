...
_docker_sdk_autocomplete() {
    ...
    opts="build start stop cli composer phpstan phpunit phpcs phpcbf hook-install hook to-generate df-generate"

    case "${prev}" in
        composer)
            # define subcommands for 'composer'
            COMPREPLY=( $(compgen -W "install update why phpunit phpstan transfer-generate definition-generate captainhook phpcs phpcbf list" -- "${cur}") )
            return 0
            ;;
        hook)
